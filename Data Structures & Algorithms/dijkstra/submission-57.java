class Solution {
    public Map<Integer, Integer> shortestPath(int n, List<List<Integer>> edges, int src) {
        ...
        // Min heap tracks edge weights lowest-to-highest
        Queue<int[]> minHeap = new PriorityQueue<>( (a, b) -> a[0] - b[0]);
        // It costs nothing to stay at the current vertex.
        minHeap.offer(new int[]{0, src});
        
        // Setup the adjacency map
        for (int i = 0; i < n; i ++){
        ...
        while (!minHeap.isEmpty()){
            ...
            for (int[] neighbor :  adj.get(n1)){
                ...
                if (!distances.containsKey(n2)){
                    // Greedy choice when exploring frontier
