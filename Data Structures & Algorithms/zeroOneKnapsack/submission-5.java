class Solution {
    public int maximumProfit(List<Integer> profit, List<Integer> weight, int capacity) {
      return maximumProfitMemoized(profit, weight, capacity);
    }

    // Time Complexity: O(N * M) because each state (i, capacity) is computed only once.
    // Space Complexity: O(N * M) for the cache, plus O(N) for the recursion stack.
    public int maximumProfitMemoized(List<Integer> profit, List<Integer> weight, int capacity) {
